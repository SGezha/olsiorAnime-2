...
onMounted(() => {
    ...
    if (anime.value != null) {
        if (Hls.isSupported()) {
            hls.loadSource(anime.value.data[0].attributes.episodes[current.value].hls)
            hls.attachMedia(document.querySelector('video'))
            if(anime.value.data[0].attributes.episodes[current.value].chat) getChat()
        } else {
            video.value.src = anime.value.data[0].attributes.episodes[current.value].hls
        }
    }
})
...
const getSecondTitle = (data) => {
    return `${data.title.split('/')[1]}, ${new Date(data.date).toLocaleDateString()}, `
}

const nowChat = useState(() => [])
watch(currentTime, (val) => {
    let now = parseInt(val)
    if(anime.value.data[0].attributes.episodes[current.value].chat) {
...
const getChat = async () => {
    fetch('https://cdn.glitch.global/513930f1-8551-4a01-b9f0-59a88e2429c1/emotes.json?v=1644150481352')
        .then(res => res.json())
        .then(async emotes => {
            let response = await fetch(anime.value.data[0].attributes.episodes[current.value].chat)
            ...
            <div class="flex flex-col pt-[30px] gap-[30px] md:gap-[50px] md:flex-row">
                ...
                <div class="relative w-full showAnimation">
                    <h1 class="font-bold text-2xl">
                        {{ anime.data[0].attributes.title.split('/')[0] }}
                    </h1>
                    <h2 class="font-regular opacity-70 text-xl">
                        {{ getSecondTitle(anime.data[0].attributes) }} {{ `${anime.data[0].attributes.maxepisodes == 1 ?
                                $t('film') : anime.data[0].attributes.nowepisode + ' ' + $t('iz') + ' ' +
                                anime.data[0].attributes.maxepisodes + ' ' + $t('ep')}`
                        }}
                    </h2>

                    <div class="mt-[10px]">
                        <div class="outline-none relative" :class="{ 'h-screen w-screen': isFullscreen }" :tabindex="0"
                            autofocus @keydown.prevent.space="playing = !playing" @keydown.right="currentTime += 10"
                            ...
                            <div class="relative bg-black rounded-md shadow overflow-hidden  flex items-center"
                                ...
                                <div class="w-full flex h-full relative">
                                    <video ref="video" crossorigin="anonymous" class="block h-100 w-[100%]" :class="{ 'min-w-[70%]' : !anime.data[0].attributes.episodes[current].chat == undefined}" :loop="loop"
                                        @click="playing = !playing" />
                                    <div v-if="anime.data[0].attributes.episodes[current].chat != undefined" class="chat w-[30%] p-[10px] overflow-y-auto overflow-x-hidden flex flex-col relative right-0">
                                ...
                                <div class="absolute bottom-0 bg-base-200 bg-opacity-50 video-menu w-[100%] p-[20px] min-w-100 h-[106px] transition-transform"
                                    ...
                                    <div class="grid grid-cols-5 md:flex md:items-center mt-[10px] gap-[10px]">
                                        ...
                                        <VideoMenu>
                                            <template #default="{ open }">
                                                <button class="btn videoBtn" @click="open()">
                                                    <svg xmlns="http://www.w3.org/2000/svg"
                                                        xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true"
                                                        role="img" class="iconify iconify--carbon" width="32"
                                                        height="32" preserveAspectRatio="xMidYMid meet"
                                                        viewBox="0 0 32 32">
                                                        <path fill="currentColor"
                                            ...
                                            <template #menu="{ close }">
                                                <div
                                                    ...
                                                    <VideoMenuItem
                                                        :class="{ 'bg-primary': controls.rate.value == 1.75 }"
                                                    ...
                                                    <VideoMenuItem
                                                        :class="{ 'bg-primary': controls.rate.value == 1.25 }"
                                                    ...
                                                    <VideoMenuItem
                                                        :class="{ 'bg-primary': controls.rate.value == 0.75 }"
                                        ...
                                        <button class="btn videoBtn" @click="toggle()">
                    ...
                    <div class="mt-[30px]">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 mt-[10px] w-full gap-[10px]">
                            <button v-for="(ep, ind) in anime.data[0].attributes.episodes" :key="ep.title" class="btn"
